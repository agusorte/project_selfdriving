...
namespace project_selfdriving {

ProjectSelfdriving::ProjectSelfdriving(ros::NodeHandle nh)
    : nodeHandle_(nh){
      
  ROS_INFO("[ProjectSelfdriving] Node started.");


  init();
}

ProjectSelfdriving::~ProjectSelfdriving(){

  kittiFile.close();
}
void ProjectSelfdriving::init(){


 ROS_INFO ("[init] project_Selfdriving_node");
 // read parameters
 readParameters();
   
}

void ProjectSelfdriving::readParameters(){
  ...
  sync->registerCallback(boost::bind(&ProjectSelfdriving::callback,this,_1,_2,_3));
...
void ProjectSelfdriving::callback(const sensor_msgs::ImageConstPtr& msg,
  ...
  ROS_INFO ("sync %d   ", cloud->points.size() );
  // images
  ...
  for (auto i:bb_msg->bounding_boxes){
  ...
  for (auto cloudi:cloudObjs){
    ...
    kittiFile<<"\'"+bb_msg->bounding_boxes[i].Class+"\'"<<" "<<1<<" "
    <<3<<" "<<angle_y<<" " // check value
    << bb_msg->bounding_boxes[i].xmin<<" "<<bb_msg->bounding_boxes[i].ymin<<" "
    << bb_msg->bounding_boxes[i].xmax<<" "<<bb_msg->bounding_boxes[i].ymax<<" "
    << minor_value<<" "<<major_value<<" "<<middle_value<<" " // check values
    << position_OBB.x<<" "<<position_OBB.y<<" "<<position_OBB.z<<" "
    << angle_y<<" "<< bb_msg->bounding_boxes[i].probability<<" \n";
...
void ProjectSelfdriving::projectPointCloud2Image( const PointCloud<PointXYZ>::Ptr &  cloud,
...
void ProjectSelfdriving::segmentPointCloud( const PointCloud<PointXYZ>::Ptr &  cloud,
...
void ProjectSelfdriving::computeBoundingBox(const PointCloud<PointXYZ>::Ptr &  cloud,
    pcl::PointXYZ& min_point_AABB,
    pcl::PointXYZ& max_point_AABB,
    pcl::PointXYZ& min_point_OBB,
    pcl::PointXYZ& max_point_OBB,
